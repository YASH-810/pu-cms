...
const email = 'editor@mes.ac.in';
const fullName = 'Editor User';
...
try {
  ...
  const role = await db('roles').select('id').where({ name: 'EDITOR' }).first();
  if (!role) {
    throw new Error('EDITOR role does not exist. Run migrations first.');
  }
  ...
  console.log(`Seeded Editor: ${user.email}`);
} catch (error) {
  console.error(error);
} finally {
  await db.destroy();
